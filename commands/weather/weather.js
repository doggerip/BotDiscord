...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        ...
        .setDescription('La localisation est')
        .setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName('jour')
        .setDescription('météo de demain')
        .setRequired(false)
        .addChoices(
  ...
  async execute (interaction) {
    ...
    try {
      ...
      // Date de la météo
      const laDate = moment(forecastHour.time)
        .locale('fr')
        .format('LLLL')
        .toString()
      ...
      const airQualityDescription =
        airQuality['us-epa-index'] <= 2 ? 'Bonne' : 'Mauvaise'
      let airQualityEmoji = ''
      ...
      for (const range in moonIlluminationEmojis) {
        const [min, max] = range.split('-')
        if (
          moonIllumination >= parseInt(min) &&
          moonIllumination <= parseInt(max)
        ) {
      ...
      // Lever du jour
      const sunrise = moment(forecastAstro.sunrise, 'hh:mm A')
        .locale('fr')
        .format('LT')

      // Couché du soleil
      const sunset = moment(forecastAstro.sunset, 'hh:mm A')
        .locale('fr')
        .format('LT')
      ...
      const alert =
        forecastAlert.alert &&
        forecastAlert.alert.alert &&
        forecastAlert.alert.alert.length > 0
          ? forecastAlert.alert.alert
          : 'Aucune alerte'
      ...
      interaction.editReply({
        content:
          "Une erreur s'est produite bordel, encore un coup des Manceaux  ."
      })
    }
  }
}
